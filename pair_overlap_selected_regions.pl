...
system "bedtools intersect -a $file.neg1.tmpbed -b $file.pos1.tmpbed -wo > $file.inters2.tmptab";
...
{#print "$_\t$totals{$_}\t";
    ...
    for (my $i=0;$i<@{$counts{$_}} ;$i++) {
       $freqs{$overlaps{$_}[$i]}+=$counts{$_}[$i]/$totals{$_}*$count1; 
    }
